    ...
    $(document).ready(function () {
        ...
        $('#btnUpd').on('click', function (e) {
            var hsId = localStorage.getItem('hsId'),
                Id = localStorage.getItem("hvacId"),
                Cod = $('#Cod_Upd').val(),
                Descr = $('#Descr_Upd').val(),
            ...
            if (chkUpd(hsId, Id, Cod, Descr) == true) {
                var req = $.DataAccess.hs_hvac_Update(Id, Cod, Descr, UserName, marcamodello, installationDate);
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        $.module.load('Impianti/Operators/Device8hvac');
        ...
        function chkUpd(hsId, Id, Cod, Descr) {
            ...
            if (!error_present) {
                var req = $.DataAccess.hs_hvac_ReadByCodSync(hsId, Cod);
                req.success(function (json) {
                    var data = json.d;
                    if (data) {
                        if (data.Id != Id) {
        ...
        $('#btnDelete').on('click', function (e) {
            ...
            setTimeout(function () {
                var req = $.DataAccess.hs_hvac_Del(localStorage.getItem("hvacId"));
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        localStorage.removeItem("hvacId");
                        $.module.load('Impianti/Operators/Device8hvac');
                        loadhs_Elem(); //posta in common.js
                    }
                    else {
