$(function () {
    $(document).ready(function () {
        ...
        function loadLog() {
            $("#logList").empty();
            var r = $.DataAccess.log_hs_Gru_List(localStorage.getItem("hsId"), localStorage.getItem("GruCod"), $('#datefrom').val(), $('#dateto').val());
            r.success(function (json) {
                var data = json.d;
                if (data) {
                    for (var i = 0; i < data.length; i++) {
                        data[i].dtLog = moment(data[i].dtLog).format('DD/MM/YYYY HH:mm');
                        $('#logDescription').text(data[i].GruCod + ' ' + data[i].GruDescr);
                    }//end for
        ...
        $.fn.call_export = function () {
            var url = $.appParms.urlGlobal() + 'hs_log_Export.ashx?hsId=' + localStorage.getItem("hsId") + '&type=S&code=' + localStorage.getItem("GruCod") + '&fromDate=' + $('#datefrom').val() + '&toDate=' + $('#dateto').val();
            window.open(url);
        }
