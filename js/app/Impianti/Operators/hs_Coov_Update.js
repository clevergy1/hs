...
$(function () {
    ...
    $(document).ready(function () {
        ...
        function ReadCoov() {
            var req = $.DataAccess.hs_Coov_Read(localStorage.getItem("CoovId"));
            req.success(function (json) {
                var data = json.d;
                if (data) {
        ...
        $('#btnUpd').on('click', function (e) {
            var hsId = localStorage.getItem('hsId'),
                Id = localStorage.getItem("CoovId"),
                Cod = $('#Cod_Upd').val(),
                Descr = $('#Descr_Upd').val(),
            ...
            if (chkUpd(hsId, Id, Cod, Descr) == true) {
                var req = $.DataAccess.hs_Coov_Update(Id, Cod, Descr, UserName, marcamodello, installationDate);
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
        ...
        $('#btnDelete').on('click', function (e) {
            ...
            setTimeout(function () {
                var req = $.DataAccess.hs_Coov_Del(localStorage.getItem("CoovId"));
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        localStorage.removeItem("CoovId");
                        $.module.load('Impianti/Operators/Device8Coov');
                        loadhs_Elem(); //posta in common.js
                    }
                    else {
