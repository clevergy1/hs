...
$(function () {
    ...
    $(document).ready(function () {
        ...
        function Read() {
            var req = $.DataAccess.hs_tb_Read(localStorage.getItem("tbId"));
            req.success(function (json) {
                var data = json.d;
                if (data) {
        ...
        $('#btnUpd').on('click', function (e) {
            ...
            if (chkUpd(hsId, Id, Cod, Desc) == true) {
                var req = $.DataAccess.hs_tb_Update(Id, Cod, Desc, UserName, marcamodello, installationDate);
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        $.module.load('Impianti/Operators/Device8tb');
        ...
        function chkUpd(hsId, Id, Cod, Desc) {
            ...
            if (!error_present) {
                var req = $.DataAccess.hs_tb_ReadByCodSync(hsId, Cod);
                req.success(function (json) {
                    var data = json.d;
                    if (data) {
                        if (data.Id != Id) {
        ...
        $('#btnDelete').on('click', function (e) {
            ...
            setTimeout(function () {
                var req = $.DataAccess.hs_tb_Del(localStorage.getItem("tbId"));
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        localStorage.removeItem("tbId");
                        $.module.load('Impianti/Operators/Device8tb');
                        loadhs_Elem(); //posta in common.js
                    }
                    else {
