    ...
    $(document).ready(function () {
        ...
        function loadParking() {
            ...
            r.success(function (json) {
                var data = json.d;
                if (data) {
                    $("#tmplStallList").tmpl(data).appendTo("#StallList");
                    $('#StallList').slimscroll({
                        height: '300px',
                        wheelStep: 35
                    });
                    setlanguage();
        ...
        $.fn.selStall = function (PlaceId) {
            ...
            var req = $.DataAccess.Parking_Stall_ReadByPlaceId(PlaceId);
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    $('#currentStall').text(data.IdStall);
                    ...
                    if ( parseFloat(data.Latitude)>0 && parseFloat(data.Longitude)>0  ){
                        ...
                        marker = new google.maps.Marker({
                            ...
                            id: data.PlaceId,
                            content: setInfowindowContent(data.IdStall, data.Latitude, data.Longitude, data.AltSLM)
                        });
        ...
        $('#bntUpd').on('click', function (e) {
            ...
            var req = $.DataAccess.Parking_Stall_setCoords(PlaceId, Latitude, Longitude, AltSLM);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadParking();
            ...
            geocoder.geocode({ 'latLng': latlng }, function (results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
                    if (results[0]) {
                        ...
                        infowindow.setContent(setInfowindowContent($('#currentStall').text(), $('#Latitude_Upd').val(), $('#Longitude_Upd').val(),  $('#AltSLM_Upd').val()));
        ...
        function setInfowindowContent(IdStall, lat, lng, alt) {
            var html = "";
            html = '<div id="content" style="display:block;white-space: nowrap;width:200px;">';
            html += '<div id="siteNotice">' + IdStall + '</div>';
