...
$(function () {
    ...
    $(document).ready(function () {
        ...
        function ReadAnz() {
            var req = $.DataAccess.cymt200_Read(localStorage.getItem("cymt200Id"));
            req.success(function (json) {
                var data = json.d;
                if (data) {
        ...
        $('#btnUpd').on('click', function (e) {
            var hsId = localStorage.getItem('hsId'),
                Id = localStorage.getItem("cymt200Id"),
            ...
            if (chkUpd(hsId, Id, Cod, sn, Descr) == true) {
                var req = $.DataAccess.cymt200_Update(Id, Cod, Descr, sn, UserName, marcamodello, installationDate, NoteInterne);
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        $.module.load('Impianti/supervisors/Device8sdincymt200');                       
                    }
                    else {
        ...
        function chkUpd(hsId, Id, Cod, sn, Descr) {
            ...
            if (!error_present) {
                var req = $.DataAccess.cymt200_ReadByCodSync(hsId, Cod);
                req.success(function (json) {
                    var data = json.d;
                    if (data) {
                        if (data.Id != Id) {
            ...
            if (!error_present) {
                var req = $.DataAccess.cymt200_ReadBysn(hsId, sn);
                req.success(function (json) {
                    var data = json.d;
                    if (data) {
                        if (data.Id != Id) {
        ...
        $('#btnDelete').on('click', function (e) {
            ...
            setTimeout(function () {
                var req = $.DataAccess.cymt200_Del(localStorage.getItem("cymt200Id"));
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        localStorage.removeItem("cymt200Id");
                        $.module.load('Impianti/supervisors/Device8sdincymt200');
                     }
                    else {
                        toastr["warning"](langResources['msg4operationfailed'], langResources['alert']);
       ...
       ---------------------------------------------------------------*/
        function loadReplacements() {
            ...
            var r = $.DataAccess.cymt200_replacement_history_List(localStorage.getItem("cymt200Id"));
        ...
        $('#btnAddReplacements').on('click', function () {
            var ParentId = localStorage.getItem('cymt200Id'),
                marcamodello = $('#marcamodello_Add').val().trim(),
                installationDate = $('#installationDate_Add').val(),
                note = $('#note_Add').val().trim(),
                userName = localStorage.getItem("SupervisorName");
            var req = $.DataAccess.cymt200_replacement_history_Add(ParentId, marcamodello, installationDate, note, userName);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
        ...
        $.fn.CallEdit = function (Id) {
            ...
            var req = $.DataAccess.cymt200_replacement_history_Read(Id);
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    $('#Id_Upd').val(data.Id);
        ...
        $('#btnUpdReplacement').on('click', function () {
            ...
            note = $('#note_Upd').val(),
            userName = localStorage.getItem("SupervisorName");
            var req = $.DataAccess.cymt200_replacement_history_Update(Id, marcamodello, installationDate, note, userName);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
        ...
        $('#btnDeleteReplacements').on('click', function () {
            var Id = $('#Id_Upd').val();
            var req = $.DataAccess.cymt200_replacement_history_Del(Id);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
