...
$(function () {
    ...
    $(document).ready(function () {
        ...
        function ReadE() {
            var req = $.DataAccess.hs_Doors_Read(localStorage.getItem("ParentId"));
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    console.log('installationDate',moment(data.installationDate).format('DD/MM/YYYY'));
                    //$(".Descr").text(data.DoorDesc);
                    $(".Descr").text(data.DoorDesc + ' (' + data.DoorCod + ')');                    
        ...
        function loadReplacements() {
            ...
            var r = $.DataAccess.hs_Doors_replacement_history_List(localStorage.getItem("ParentId"));
        ...
        $('#btnAdd').on('click', function () {
            ...
            var req = $.DataAccess.hs_Doors_replacement_history_Add(ParentId, marcamodello, installationDate, note, userName);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
        ...
        $.fn.CallEdit = function (Id) {
            ...
            var req = $.DataAccess.hs_Doors_replacement_history_Read(Id);
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    $('#Id_Upd').val(data.Id);
        ...
        $('#btnUpd').on('click', function () {
            ...
            note = $('#note_Upd').val(),
            userName = localStorage.getItem("SupervisorName");
            var req = $.DataAccess.hs_Doors_replacement_history_Update(Id, marcamodello, installationDate, note, userName);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
        ...
        $('#btnDelete').on('click', function () {
            var Id = $('#Id_Upd').val();
            var req = $.DataAccess.hs_Doors_replacement_history_Del(Id);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
        ...
        $('#btnCloseUpdParent').on('click', function () {
            $.module.load('Impianti/supervisors/Device8Door');
            //$('#updateParent').hide();
            //$('#update').hide();
            //$('#add').hide();
            //$('#list').show();
        ...
        $('#btnUpdParent').on('click', function () {
            var marcamodello = $('#marcamodello_UpdParent').val(),
                ...
                userName = localStorage.getItem("SupervisorName");
            var req = $.DataAccess.cymt100_UpdateMarcamodello(localStorage.getItem("ParentId"), marcamodello, installationDate, userName);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    $('#parentRecord').show();
