/*
Impianti supervisors Manage Heating system tickets
------------------------------------------*/
$(function () {
    ...
    $(document).ready(function () {
        ...
        function loadRequester() {
            $("#TicketRequesterList").empty();
            var req = $.DataAccess.hs_Tickets_Requesters_List(localStorage.getItem("hsId"));
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    $("#tmpTicketRequesterList").tmpl(data).appendTo("#TicketRequesterList");
                    $("#TicketRequesterList").niceScroll({
                        cursorcolor: "#1FB5AD",
        ...
        function loadExecutor() {
            $("#TicketExecutorList").empty();
            var req = $.DataAccess.hs_Tickets_Executors_List(localStorage.getItem("hsId"));
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    $("#tmpTicketExecutorList").tmpl(data).appendTo("#TicketExecutorList");
                    $("#TicketExecutorList").niceScroll({
                        cursorcolor: "#1FB5AD",
                        cursorborder: "0px solid #fff",
                        cursorborderradius: "0px",
                        cursorwidth: "3px"
        ...
        $('#Requester_Add').on('focus', function (e) {            
            $("#ModalSelectRequester").modal('show');
        });
        $('#Requester_Upd').on('focus', function (e) {
            $("#ModalSelectRequester").modal('show');
        })
        $('#Executor_Add').on('focus', function (e) {
            $("#ModalSelectExecutor").modal('show');
        });
        $('#Executor_Upd').on('focus', function (e) {
            $("#ModalSelectExecutor").modal('show');
        })

        $.fn.selTicketRequester = function (Id, Nome) {
            $('#Requester_Add').val(Nome);
            $('#Requester_Upd').val(Nome);
            $('#emailRequester_Add').val('');
            $('#emailRequester_Upd').val('');
            var req = $.DataAccess.hs_Tickets_Requesters_Read(Id);
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    console.log('hs_Tickets_Requesters_Read', data);
                    $('#emailRequester_Add').val(data.emailaddress);
                    $('#emailRequester_Upd').val(data.emailaddress);
                }
            });
            $("#ModalSelectRequester").modal('hide');
        }

        $.fn.selTicketExecutor = function (Id, Nome) {
            console.log('$.fn.selTicketExecutor', Id, Nome);
            $('#Executor_Add').val(Nome);
            $('#Executor_Upd').val(Nome);
            $('#emailExecutor_Upd').val('');
            $('#emailExecutor_Add').val('');
            var req = $.DataAccess.hs_Tickets_Executors_Read(Id);
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    console.log('hs_Tickets_Executors_Read', data);
                    $('#emailExecutor_Add').val(data.emailaddress);
                    $('#emailExecutor_Upd').val(data.emailaddress);
                }
            });
            $("#ModalSelectExecutor").modal('hide');
        }
        /*---------------------------------------------------------------*/
        ...
        $('#manage_people').on('click', function (e) {
            $.module.load('Impianti/Supervisors/hs_Tickets_people');
        });
