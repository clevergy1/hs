    ...
    $(document).ready(function () {
        ...
        $('#btnUpd').on('click', function (e) {
            ...
            if (chkUpd(hsId, Id, Cod, Descr) == true) {
                var req = $.DataAccess.hs_spl_Update(Id, Cod, Descr, NoteInterne, UserName, marcamodello, installationDate);
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
                        $.module.load('Impianti/supervisors/Device8Spl');
        ...
        function chkUpd(hsId, Id, Cod, Descr) {
            ...
            if (!error_present) {
                var req = $.DataAccess.hs_spl_ReadByCod_Sync(hsId, Cod);
                req.success(function (json) {
                    var data = json.d;
                    if (data) {
                        if (data.Id != Id) {
        ...
        $('#btnDelete').on('click', function (e) {
            ...
            setTimeout(function () {
                var req = $.DataAccess.hs_spl_Del(localStorage.getItem("SplId"));
                req.success(function (json) {
                    var data = json.d;
                    if (data == true) {
        ...
        function loadReplacements() {
            ...
            var r = $.DataAccess.hs_spl_replacement_history_List(localStorage.getItem("SplId"));
        ...
        $('#btnAddReplacements').on('click', function () {
            var ParentId = localStorage.getItem('SplId'),
                marcamodello = $('#marcamodello_Add').val().trim(),
                installationDate = $('#installationDate_Add').val(),
                note = $('#note_Add').val().trim(),
                userName = localStorage.getItem("SupervisorName");
            var req = $.DataAccess.hs_spl_replacement_history_Add(ParentId, marcamodello, installationDate, note, userName);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
        ...
        $.fn.CallEdit = function (Id) {
            ...
            var req = $.DataAccess.hs_spl_replacement_history_Read(Id);
            req.success(function (json) {
                var data = json.d;
                if (data) {
                    $('#Id_Upd').val(data.Id);
        ...
        $('#btnUpdReplacement').on('click', function () {
            ...
            note = $('#note_Upd').val(),
            userName = localStorage.getItem("SupervisorName");
            var req = $.DataAccess.hs_spl_replacement_history_Update(Id, marcamodello, installationDate, note, userName);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
        ...
        $('#btnDeleteReplacements').on('click', function () {
            var Id = $('#Id_Upd').val();
            var req = $.DataAccess.hs_spl_replacement_history_Del(Id);
            req.success(function (json) {
                var data = json.d;
                if (data == true) {
                    loadReplacements();
