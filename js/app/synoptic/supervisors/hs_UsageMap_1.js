    ...
    $(document).ready(function () {
            ...
            r.success(function (json) {
                ...
                if (data) {
                   // console.log(data);
                    ...
                    $('#PlantList').slimscroll({height: '315px',wheelStep: 15});
                    setlanguage();
      ...
      ----------------------------------------------------------------*/
        ...
        $.fn.Cal = function (hsId) {
            $("#PlantList").empty();
            localStorage.setItem("Cod", "Cal");
            var r = $.DataAccess.hs_Cal_List(localStorage.getItem("hsId"));
            r.success(function (json) {
                var data = json.d;
                if (data) {
                    console.log("CALDAIE", data);
                    data.installationDate = moment(data.installationDate).format('DD/MM/YYYY');
                    $("#tmplCal").tmpl(data).appendTo("#PlantList");
       ...
       ----------------------------------------------------------------*/
        $.fn.Ev = function (hsId) {
            $("#PlantList").empty();
            localStorage.setItem("Cod", "Ev");
            var r = $.DataAccess.hs_Ev_List(localStorage.getItem("hsId"));
            r.success(function (json) {
                var data = json.d;
                if (data) {
                    console.log("ELETROVALVOLE", data);
                    data.installationDate = moment(data.installationDate).format('DD/MM/YYYY');
                    $("#tmplEvDetail").tmpl(data).appendTo("#PlantList");
      ...
      //------------------------------------------------------------------------*/
        ...
        function loadErrorLog() {
            ...
            r.success(function (json) {
                ...
                if (data) {
                    ...
                    $('#panel-body-ListLog').slimscroll({ height: '315px', wheelStep: 15 });
                } 
            });
        }
        ...
        $("#panel-body-ListLog").mousedown(function (event) {
            event.stopPropagation();
...
function selSynoptic(MapId) {
    $.fn.selSynoptic(MapId);
}
